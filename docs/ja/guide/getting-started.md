...
Shekereは **Three.jsをベースとした独自のAPI** を提供しています。自分でレンダラーをセットアップする代わりに、Shekereが呼び出す特定のライフサイクル関数をエクスポートすることでビジュアルを定義します。

以下のコードを `my_first_sketch.js` として保存してください。
...
次に、Shekereを起動して作成したファイルを読み込みます：
1. Shekereを起動します。「コントロールパネル」と「ビジュアライザー」の2つのウィンドウが表示されます。
2. **Control Panel** ウィンドウで、**"Open Sketch"** ボタンをクリックします。
3. 先ほど作成した `my_first_sketch.js` を選択します。
4. これで、あなただけの3Dオーディオ・リアクティブ・ビジュアルの完成です！

---

### さらにサンプルを探すには？
GitHubリポジトリの [examples/](https://github.com/katk3n/shekere/tree/main/examples) ディレクトリには、MIDIやOSC、ポストプロセス制御などをカバーしたより高度なサンプルスクリプトが用意されています。
