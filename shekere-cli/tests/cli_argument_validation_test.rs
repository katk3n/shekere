...
fn test_cli_help_flag_works() {
    ...
    assert!(
        help_text.contains("shekere"),
        "Help text should contain program name"
    );
    assert!(
        help_text.contains("FILE") || help_text.contains("config"),
...
fn test_cli_short_help_flag_works() {
    ...
    let help_text = String::from_utf8_lossy(&help_output.stdout);
    assert!(
        help_text.contains("USAGE") || help_text.contains("Usage") || help_text.contains("shekere"),
...
fn test_cli_version_flag_works() {
    ...
    assert!(
        ...
        "Version text should contain binary name 'shekere-cli', got: {}",
        version_text
    );

    // Check version format (should contain version number)
    assert!(
        version_text.contains("0.13.0") || version_text.chars().any(|c| c.is_numeric()),
        "Version text should contain version number, got: {}",
        version_text
    );
...
fn test_cli_short_version_flag_works() {
    ...
    let version_text = String::from_utf8_lossy(&version_output.stdout);
    assert!(
        version_text.contains("shekere-cli") || version_text.chars().any(|c| c.is_numeric()),
...
fn test_cli_rejects_invalid_flags() {
    ...
    assert!(
        ...
        "Error should indicate unrecognized flag, got: {}",
        stderr
    );
}
...
fn test_cli_requires_config_file_argument() {
    ...
    assert!(
        stderr.contains("required") || stderr.contains("FILE") || stderr.contains("argument"),
        "Error should indicate missing required argument, got: {}",
        stderr
    );
}
...
fn test_cli_handles_multiple_arguments_properly() {
    ...
    if !output.status.success() {
        ...
        assert!(
            ...
            "Should handle multiple arguments gracefully, got: {}",
            stderr
        );
    }
}
...
fn test_cli_help_mentions_supported_features() {
    ...
    assert!(
        help_text.contains("config")
            || help_text.contains("shader")
            || help_text.contains("visual"),
        "Help should give some indication of what shekere does, got: {}",
        help_text
    );
}
