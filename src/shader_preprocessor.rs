...
impl ShaderPreprocessor {
    ...
    pub fn process_file_with_embedded_defs(&self, file_path: &Path) -> Result<String, PreprocessorError> {
        // Read user shader file
        let user_content = std::fs::read_to_string(file_path).map_err(|e| {
...
mod tests {
    ...
    fn test_process_file_with_embedded_defs() {
        ...
        create_test_file(dir_path, "user.wgsl", "@fragment\nfn fs_main() -> @location(0) vec4<f32> {\n    return vec4(1.0);\n}");

        let preprocessor = ShaderPreprocessor::new(dir_path);
        let result = preprocessor.process_file_with_embedded_defs(&dir_path.join("user.wgsl")).unwrap();
        ...
        assert!(embedded_pos < user_pos, "Embedded definitions should come before user code");
    }
    ...
    fn test_file_not_found() {
        ...
        let preprocessor = ShaderPreprocessor::new(dir_path);
        let result = preprocessor.process_file_with_embedded_defs(&dir_path.join("nonexistent.wgsl"));

        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), PreprocessorError::FileRead { .. }));
    }
    ...
    fn test_embedded_defs_content() {
        ...
        let preprocessor = ShaderPreprocessor::new(dir_path);
        let result = preprocessor.process_file_with_embedded_defs(&dir_path.join("simple.wgsl")).unwrap();
