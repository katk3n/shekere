...
mod tests {
    ...
    fn test_mock_hot_reloader_initial_state() {
        ...
        assert!(!reloader.check_for_changes(), "Should start with no changes");
    }
    ...
    fn test_mock_hot_reloader_simulate_change() {
        ...
        assert!(reloader.check_for_changes(), "Should detect simulated change");
        
        // Should reset after checking
        assert!(!reloader.check_for_changes(), "Should reset after first check");
    }
    ...
    fn test_hot_reloader_with_valid_file() {
        ...
        assert!(result.is_ok(), "Should successfully create HotReloader for valid file");
    }

    #[test]
    fn test_hot_reloader_with_invalid_path() {
        let result = HotReloader::new("/non/existent/path/shader.wgsl");
        assert!(result.is_err(), "Should fail to create HotReloader for invalid path");
    }
    ...
    fn test_hot_reloader_check_for_changes_resets_flag() {
        ...
        assert!(reloader.check_for_changes(), "First check should detect change");
        
        // Subsequent checks should return false
        assert!(!reloader.check_for_changes(), "Second check should not detect change");
        assert!(!reloader.check_for_changes(), "Third check should not detect change");
    }
}