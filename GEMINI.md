...
4. Verify (検証):
    - 実装完了後、直ちにシンタックスエラーや閉じ括弧の欠落等がないかを最優先で確認すること。
...
- **Git Push の制限**: `git push`（タグを含む）を実行する際は、必ず事前に具体的なコミット内容やタグ名を提示し、ユーザーから明示的な「Approve（承認）」を得ること。勝手にリモートリポジトリへプッシュしてはならない。
