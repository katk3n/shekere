...
1. **事前確認**: 全ての機能実装、バグ修正、ドキュメント更新がコミットされており、`git status` がクリーンであることを必ず確認すること。未コミットの変更がある状態でバージョン更新作業を始めてはならない。
2. `package.json` の `"version"` を更新する
3. `tauri.conf.json` の `"version"` を更新する（`package.json` と同じ値にすること）
4. `Cargo.toml` の `version = "..."` を更新する（`package.json` と同じ値にすること）
5. 変更をコミットする
6. バージョンタグを打つ: `git tag v<バージョン番号>`
7. main ブランチとタグをプッシュする: `git push origin main v<バージョン番号>`

**Git Push の注意**: 手順3.4（ターミナルルール）に基づき、ステップ7を実行する前には必ずコミット内容とタグ名を提示し、ユーザーからの明示的な承認を得ること。
